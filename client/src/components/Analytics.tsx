...
export default function Analytics({
  ...
  const getIRIStatus = (score: number) => {
    ...
    if (score >= 50) return { label: 'INTERVIEW READY', color: 'text-accent-blue', desc: 'Ready to submit OA assessment roles.' };
    ...
    <div className="flex flex-col gap-6 animate__animated animate__fadeIn">
      ...
      <div className="brutal-card p-8 border-3 border-border shadow-[6px_6px_0px_var(--shadow-color)] bg-bg-surface relative">
        ...
        <div className="flex justify-between items-center">
          <div>
            <h2 className="brutal-title text-2xl font-black m-0 leading-none text-text-primary">INTERNSHIP READINESS INDEX</h2>
            <p className="brutal-mono text-xs text-text-secondary mt-2">
              Aggregate SDE metric weighting DSA, Systems code, and Capstone projects.
            </p>
          </div>
          <span className="brutal-title text-3xl font-black text-text-primary">{internshipReadinessIndex}%</span>
        </div>

        <div className="w-full h-5 border-3 border-border bg-bg-surface-alt relative my-6">
          <div
        ...
        <div className="flex justify-between items-center text-xs font-bold uppercase text-text-primary">
          <span>Status: <b className={iri.color}>{iri.label}</b> ({iri.desc})</span>
          <span className="brutal-mono text-text-secondary">Target: 90%</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-6 flex flex-col gap-6">
          <div className="brutal-card p-6 border-3 border-border shadow-[6px_6px_0px_var(--shadow-color)] bg-bg-surface relative">
            ...
            <h2 className="brutal-title text-base font-black mb-4 text-text-primary">DOMAIN MASTERY</h2>
            ...
            <div className="flex flex-col gap-4">
              {[
                ...
                <div key={idx}>
                  <div className="flex justify-between text-xs font-bold uppercase mb-1 text-text-primary">
                    <span>{item.label}</span>
                    <span>{item.val}%</span>
                  </div>
                  <div className="w-full h-3 border-2 border-border bg-bg-surface-alt">
        ...
        <div className="lg:col-span-6 flex flex-col gap-6">
          <div className="brutal-card p-6 border-3 border-border shadow-[6px_6px_0px_var(--shadow-color)] bg-bg-surface relative">
            ...
            <div className="flex justify-between items-center mb-4">
              <h2 className="brutal-title text-base font-black m-0 text-text-primary">CP CONTESTS</h2>
              <button
                className="brutal-btn py-1 px-3 text-[10px] bg-accent-purple text-text-primary font-bold border-2 border-border"
            ...
            {showAddContest && (
              <form onSubmit={handleLogContest} className="border-3 border-border p-4 bg-bg-surface-alt mb-4 flex flex-col gap-3">
                <div className="flex flex-col gap-1">
                  <label className="text-[9px] font-extrabold uppercase text-text-primary">Contest Name</label>
                  <input type="text" value={cName} onChange={e => setCName(e.target.value)} required className="brutal-input text-xs p-2" />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="flex flex-col gap-1">
                    <label className="text-[9px] font-extrabold uppercase text-text-primary">Platform</label>
                  ...
                  <div className="flex flex-col gap-1">
                    <label className="text-[9px] font-extrabold uppercase text-text-primary">Rating Delta</label>
                    <input type="number" value={cRating} onChange={e => setCRating(e.target.value)} placeholder="+15" className="brutal-input text-xs p-2" />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="flex flex-col gap-1">
                    <label className="text-[9px] font-extrabold uppercase text-text-primary">Problems Solved</label>
                    <input type="number" value={cSolved} onChange={e => setCSolved(e.target.value)} className="brutal-input text-xs p-2" />
                  </div>
                  <div className="flex flex-col gap-1">
                    <label className="text-[9px] font-extrabold uppercase text-text-primary">Global Rank</label>
            ...
            {contestAttempts.length === 0 ? (
              ...
              <div className="flex flex-col gap-2">
                {contestAttempts.map(att => {
                  ...
                  return (
                    <div key={att.id} className="border-2 border-border p-3 bg-bg-surface-alt flex justify-between items-center hover:translate-x-0.5 transition-all text-text-primary">
                      <div>
                        <h4 className="brutal-title text-xs font-black m-0 text-text-primary">{matchContest?.name || 'Contest Round'}</h4>
                        <span className="brutal-mono text-[9px] text-text-secondary block mt-0.5">{matchContest?.platform} · {matchContest?.date}</span>
                      </div>
                      <div className="text-right">
                        <span className="brutal-pill bg-bg-surface border border-border text-[10px] font-black text-text-primary">
                        ...
                        {att.rank && <div className="text-[9px] brutal-mono text-text-secondary">Rank: #{att.rank}</div>}
                      </div>
                    </div>
                  );
                })}
